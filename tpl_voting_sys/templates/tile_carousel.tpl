...
{counter assign='pos'}
    <div class="col-md-4" style="perspective: 1000px;">
      ...
      <div class="card carousel-tile-card name{$pos} bg-transparent h-100">
              {if !empty($value.link)}
              <a href="{$value.link}" class="stretched-link" aria-label="Przeczytaj więcej o {$value.header}"></a>
              {/if}
        ...
        <div class="card-body pt-0 pb-0 px-4">
          ...
          <h4 class="card-title position-relative">{$value.header}!</h4>
            ...
            <div class="col-12 mt-auto">
              <a href="{$value.link}" class="mt-auto btn btn-sm position-relative {$value.button_kind} dark_heading_color stretched-link"  aria-label="Przeczytaj więcej o {$value.header}">czytaj to!</a>
            </div>
              {/if}
